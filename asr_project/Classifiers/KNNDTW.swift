    ...
    private func dtw_cost(y: [Float], x: [Float]) -> Float {
        ...
        for i in 1...y.count-1 {
            for j in 1...x.count-1 {
                let cost1 = acc_cost[i-1][j-1]
                let cost2 = acc_cost[i-1][j]
                let cost3 = acc_cost[i][j-1]
                acc_cost[i][j] = min(cost1, cost2, cost3) + distances[i][j]
        ...
        while (i > 0 && j > 0) {
            if (i==0) {
                ...
                let cost2 = acc_cost[i-1][j]
                let cost3 = acc_cost[i][j-1]
                if ( acc_cost[i-1][j] == min(cost1, cost2, cost3) ) {
                    i = i - 1
                }
                else if (acc_cost[i][j-1] == min(cost1, cost2, cost3)) {
                    j = j - 1
                }
                else {
                    i = i - 1
