...
//    std::string processFrame(float* samples, size_t N) {
    double* processFrame(float* samples, size_t N) {
...
//        float* mfcc_floats = new float[mfcc.size()];
//        int i;
//        for(i=0; i<mfcc.size()-1; i++) {
//            mfcc_floats[i] = mfcc[i];
//        }
        return mfcc.data();
    }
    double* v_d_to_doubles(v_d mfcc_vector) {
        double* mfccs = new double[mfcc_vector.size()];
        for(int i=0; i < mfcc_vector.size(); i++){
            mfccs[i] = mfcc_vector[i];
        }
        return mfccs;
    }
    float* process_floats(float* samples, int size) {
        uint16_t buffer_length = winLengthSamples - frameShiftSamples;
        
        for(int i=0; i < buffer_length; i++) {
            prevsamples[i] = samples[i];
        }
//        delete(samples); // might be a bug if still needed after its passed
        uint16_t buffer_len = frameShiftSamples;
        
        auto samples_v = std::vector<float>(samples, samples + size-1);
        for(int i=buffer_length; i <= size-buffer_len-1; i+=buffer_len) {
            std::vector<float> frame(samples_v.begin()+ i, samples_v.begin()+i+buffer_len);
            double* x = processFrame(frame.data(), buffer_len);
